...
def handle_invalid_input(input_question: str, checklist: Any) -> str:
    """
    ...
    """
    choice: str = input(input_question).lower().strip()
...
def load_data(city: str, month: str, day: str) -> pd.DataFrame:
    ...
    """
    # load the city data
    df: Any = pd.read_csv(CITY_DATA[city.lower()])
...
def display_data(df: pd.DataFrame, display_stats: Any) -> None:
    buffer: int = 0
...
def main() -> None:
    while True:
        filtered_data: tuple = get_filters()
